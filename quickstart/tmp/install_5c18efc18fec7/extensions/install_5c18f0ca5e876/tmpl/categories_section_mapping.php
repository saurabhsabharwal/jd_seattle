...
<table class="w100p sortable-area table-column-map table-bordered table-drag bg-color-white">
	...
	for ($row = 0; $row < $countC; $row++)
	{
		...
		<tr class="column-map">
			...
			<?php if ($row == 0): ?>
				<td rowspan="<?php echo max($countH, $countC + $countE) ?>" class="v-top no-hover headers-cell">
	...
	?>
	<tr style="background: #999999"><td colspan="2"><hr style="margin: 1px;"/></td></tr>
	<tr class="column-map">
		<td data-column="specification_fields"><?php echo JText::_('PLG_SYSTEM_SELLACIOUSIMPORTER_IMPORT_CATEGORIES_SPECIFICATIONS_FIELDS_LABEL'); ?></td>
		<td>
			<ul class="sortable-group alias-drop"
				data-column="specification_fields" data-multiple="true"></ul>
		</td>
	</tr>
	<?php
	if ($countC + $countE < $countH)
	{
		?><tr><td colspan="2" rowspan="<?php echo $countH - ($countC + $countE) ?>">&nbsp;</td></tr><?php
	}
	?>
	</tbody>
</table>
