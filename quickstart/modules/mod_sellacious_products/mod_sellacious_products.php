...
JLoader::register('ModSellaciousProducts', __DIR__ . '/helper.php');
...
$products = $helper->product->getModProducts($params, 'products');
...
if (!empty($splList))
{
	foreach ($splList as $spl)
	{
		...
		$styles[$spl->id] = ".mod-sellacious-products .spl-cat-$spl->id { $style }";
	}
...
require JModuleHelper::getLayoutPath('mod_sellacious_products', $layout);
