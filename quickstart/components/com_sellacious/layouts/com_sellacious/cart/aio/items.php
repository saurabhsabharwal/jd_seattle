	...
	foreach ($items as $uid => $item)
	{
		...
		<tr>
			...
			<td class="cart-item">
				...
				<?php endif; ?>
				<br><div><?php echo JText::_('COM_SELLACIOUS_CART_ITEM_SOLD_BY'); ?> <span><?php echo $item->getProperty('seller_store') ? $item->getProperty('seller_store') : ($item->getProperty('seller_name') ? $item->getProperty('seller_name') : ($item->getProperty('seller_company') ? $item->getProperty('seller_company') : $item->getProperty('seller_username'))); ?></span></div>
